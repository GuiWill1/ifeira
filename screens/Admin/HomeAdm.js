...
export default class HomeAdm extends Component {
  ...
  getItensAprovados() {
    ...
    db.collection("Pedidos").orderBy("dataHora", 'desc').onSnapshot(snapshot => {
      ...
      console.log("pedidos aprovados:", this.state.pedidosAprovados.length)
  ...
  getItensNovos() {
    ...
    db.collection("Pedidos").where("finalizado", '==', false).orderBy("dataHora", 'asc').onSnapshot(snapshot => {
      ...
      console.log("pedidos novos:", this.state.pedidos.length)
      ...
      return (
        <Content>
          ...
          <View style={{ backgroundColor: '#3b5', borderRadius: 8, padding: 3, marginLeft: 3, alignItems: 'center' }}>
            { item.finalizado &&
              <Text style={{ color: '#fff', fontWeight: 'bold' }}>Produto Entregue</Text>
            }
            { !item.finalizado &&
              <Text style={{ color: '#fff', fontWeight: 'bold' }}>Aguardando Entrega</Text>
            }
